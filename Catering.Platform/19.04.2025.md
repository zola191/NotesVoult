...
3. Фильтры исключений Exception Filters (точечная обработка например обработка ) унаследована от ASP MVC
```csharp
...
Middleware > Exception Filters для глобальных ошибок