...
```
- Финализатор в C# — это специальный метод, который вызывается сборщиком мусора (Garbage Collector, GC) перед тем, как объект будет окончательно удален из памяти.
